...
{
    public class MyConsumerServiceSelector : IConsumerServiceSelector
    {
            ...
            IServiceProvider provider)
        {
                ...
                .Where(o => o.ImplementationType != null || o.ImplementationFactory != null))
            {
                ...
                if (!capSubscribeTypeInfo.IsAssignableFrom(detectType)
                    && !detectType.HasImplementedRawGeneric(typeof(DistributedEventHandler<>))
                    && !detectType.HasImplementedRawGeneric(typeof(DistributedEventHandler<,>)))
                {
                    continue;
                }
        ...
        {
            if (typeInfo.HasImplementedRawGeneric(typeof(DistributedEventHandler<>)) || typeInfo.HasImplementedRawGeneric(typeof(DistributedEventHandler<,>)))
            {
                var argumentType = typeInfo.BaseType.GetGenericArguments()[0];
                var topicClassAttribute = argumentType.GetCustomAttribute<TopicAttribute>() ?? new CapSubscribeAttribute(argumentType.ToString());
                SetSubscribeAttribute(topicClassAttribute);

                var method = typeInfo.GetRuntimeMethods()
                    .Where(a => a.Name == "HandleEventAsync")
                    .Where(a =>
                    {
                        if (typeInfo.BaseType.HasImplementedRawGeneric(typeof(DistributedEventHandler<>)))
                            return a.DeclaringType != typeof(DistributedEventHandler<>);
                        if (typeInfo.BaseType.HasImplementedRawGeneric(typeof(DistributedEventHandler<,>)))
                            return a.DeclaringType != typeof(DistributedEventHandler<,>);
                        return false;
                    })
                ...
                if (method != null)
                {
                    ...
                    yield return InitDescriptor(topicClassAttribute, method, typeInfo, serviceTypeInfo, parameters);
                }
            }
            else
            {
